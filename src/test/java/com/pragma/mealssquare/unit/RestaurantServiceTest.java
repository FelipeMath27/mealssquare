...
public class RestaurantServiceTest {
    ...
    void test_create_restaurant(){
        useCaseRestaurant.saveRestaurants(restaurant,creatorUser,ownerUser);
        verify(iRestaurantPersistencePort,times(1)).saveRestaurant(any(Restaurant.class));
    }

    @Test
    void test_create_restaurant_two() {
        assertDoesNotThrow(() -> useCaseRestaurant.saveRestaurants(restaurant, creatorUser, ownerUser));
        verify(iRestaurantPersistencePort, times(1)).saveRestaurant(any(Restaurant.class));
    }
